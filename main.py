...
if(token == ''):
    print("You don't have a Telegram Bot API token, insert one in 'API_KEYS.py' (you can get one here: https://t.me/botfather)")
if(keys_list[0] == 'YOUR_API_KEY_1' or (keys_list[1] == 'YOUR_API_KEY_2')):
    print("You don't have any 'the-odds-api' APIs token, insert one in 'API_KEYS.py' (you can get one here: https://the-odds-api.com/#get-access)")
...
oddsFormat = ['decimal']
# I will only use these two type of bets to make it easier, beacuse they are binary bets (win/lose, over/under), you can find bets explaination in 'utility.md'
markets = ['h2h', 'totals']
...
# only matches with al least 2% profit will be shown on output
PROFIT_PERCENTAGE_UPPER_LIMIT = 2
EURO_STARTING_BALANCE = input(
    'Insert your starting balance in € (-1 to start with 10000€): ')
...
print('Bot running ...')
send_message(bot, chat_id, 'BOT START')
sleep(10)
    ...
    while(i_key in range(0, len(keys_list))):
        if((int(datetime.now().strftime("%-H")) != curr_hour) and (int(datetime.now().strftime("%-H")) % HOUR_INTERVAL == 0)):
            ...
            for gain in list(map(lambda x: x['EURO_back'], profittable_matches_bet_list)):
                EURO_BALANCE += gain
            logger.info('BALANCE UPDATE -> current balance: ' +
        ...
        if(remaning_requests(response) <= 0):
            ...
            response = get_API_response_Odds(
                keys_list[i_key], regions[0], markets[1])
            ...
            for match in matches_full:
                ...
                for bookmaker in match['bookmakers']:
                    bookmaker_totals = bookmaker_totals_odds_shortner(
                        bookmaker)
                    totals_odds.append(bookmaker_totals)
                upcoming_matches_element = mathches_totals_odds_shortner(
                    match, totals_odds)
                    ...
                    for points_key, points in match['points'].items():
                        for points_elem in points:
                            ...
                            if(is_bookmakers_combinations_profitable(points_elem) and (bet_info['key'] not in list(map(lambda x: x['key'], profittable_matches_bet_list)))):
                                bet_profit_percentage = get_profits_percentage(
                                    points_elem['totals']['Over_1'], points_elem['totals']['Under_2'])
                                if(bet_profit_percentage > PROFIT_PERCENTAGE_UPPER_LIMIT):
                                    ...
                                    stake_A = get_stake_from_quote(
                                        points_elem['totals']['Over_1'], WIN_AMOUNT)
                                    stake_B = get_stake_from_quote(
                                        points_elem['totals']['Under_2'], WIN_AMOUNT)
                                    EURO_BALANCE -= stake_A + stake_B
                                    text_match = create_text(
                                        match, points_key, points_elem, stake_A, stake_B, WIN_AMOUNT)
                                    write_to_file_profittable_matches(
                                        text_match)
                                    ...
                                    logger.info('MATCH FOUND: ' +
                                                '\n\n' + text_match)
                                    logger.info(
                                        'FOUND MATCHES AMOUNT: ' + str(profittable_matches_count))

                                    # telegram message
                                    text_current_balance = '\n Current balance: ' + \
                                        str(round(EURO_BALANCE, 2)) + '€'
                                    send_message(bot, chat_id, text_match + text_current_balance)
                                    sleep(10)
                            else:
            ...
            logger.info('END API fetch N: ' + str(API_response_fetch) +
                        ', balance: ' + str(round(EURO_BALANCE, 2)) + '€')
            i_key += 1
            sleep(five_min)
    i_key = 0
